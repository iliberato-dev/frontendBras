...
async function updateDetailedSummaryChart() {
    ...
    try {
        ...
        const chartOptions = {
            ...
            plugins: {
                title: { display: true, text: 'Proporção', font: { size: 16 } },
                datalabels: {
        ...
        let chartData, chartLabels, chartTitle, summaryHtml;

        if (selectedMemberName) {
            // --- LÓGICA PARA VISÃO INDIVIDUAL ---
            const totalPresences = presencesDetails[selectedMemberName]?.totalPresencas || 0;
            const totalAbsences = absencesDetails[selectedMemberName]?.totalFaltas || 0;
            const totalMeetingDays = absencesResponse.totalMeetingDays || (totalPresences + totalAbsences);
        ...
        // Desenha o gráfico
        chartOptions.plugins.title.text = chartTitle; // Atualiza o título do gráfico dinamicamente
        ...
        myChart = new Chart(pieCtx, {
            ...
            plugins: [ChartDataLabels] // ATIVAÇÃO DO PLUGIN PARA AMBOS OS CASOS
        });
